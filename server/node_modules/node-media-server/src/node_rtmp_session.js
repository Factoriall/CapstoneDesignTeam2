...
// child process for python file execute
...
//	rtmpConnection.connect();
  ...
  rtmpVideoHandler() {
    ...
    if (this.videoCodec == 0) {
      ...
      //실제 파이썬 파일이 실행되는 부분
	  // python3 comparePose.py rtmp://3.144.208.159/live/userName -> argv[1] = 'rtmp://3.144.208.159/live/userName'
      const python = require('child_process').exec('python3 comparePose_.py ' 
      + "rtmp://3.144.208.159" + this.publishStreamPath)

	  // distinguish userName from URL
      var split = this.publishStreamPath.split('/'); 
 	  var userName = split[2];

	  // URL에 userName+품새 형태로 전달되기 때문에 userName, 품새 분리
	  userName = userName.split('+');
	  var userStatus = userName[1];
	  ...
	  // python 파일에서 output이 나왔을 때 실행되는 코드
      python.stdout.on('data', function(data) {
        console.log('outdata: ' + data.toString() + ', userName: ' + userName[0] + ', userStatus: ' + userStatus);
	    var result = data.toString();
		// update result to user database
		var sql = 'update user set result=? where userName=?';
		connection.query(sql, [result, userName[0]], function(err, result2){
		    if(err){
				console.log(err);
		    }
		    else{
				console.log('update result to user: ' + userName[0]);
					// if result->success, update pass/fail to database(final result)
					if(result == "success"){
					console.log("pass update start");
					sql = 'select pass from user where userName=?';
					connection.query(sql, userName[0], function(err, result3){
						if(err){
							console.log(err);
						}
						else{
							// 기존 유저의 pass/fail이 fail인 상태면 pass로 업데이트, pass라면 그냥 놔둠
							if(result3[0].pass == 'fail'){
								sql = 'update user set pass=? where userName=?';
								connection.query(sql, ['pass', userName[0]], function(err, result){
									if(err)
										console.log(err);
									else{
										console.log('update pass to user: ' + userName[0]);
									}
								});
							}
						}
      ...
      python.stderr.on('data', function(data) {
        console.error('errdata: ' + data.toString());
      });
